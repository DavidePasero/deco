...
def metric(mask, pred, back=True):
    """
    Compute mean IoU between prediction and mask
    
    Args:
        mask: Ground truth mask tensor
        pred: Predicted mask tensor
        back: Whether to include background in IoU calculation
        
    Returns:
        Mean IoU value
    """
    # Convert predictions to binary masks using threshold of 0.5
    pred_binary = (pred >= 0.5).float()
    
    # Calculate IoU for each batch
    batch_size = mask.shape[0]
    iou_values = torch.zeros(batch_size, device=mask.device)
    
    for b in range(batch_size):
        # Calculate intersection and union
        intersection = torch.logical_and(pred_binary[b], mask[b]).sum()
        union = torch.logical_or(pred_binary[b], mask[b]).sum()
        
        # Avoid division by zero
        if union > 0:
            iou_values[b] = intersection / union
        else:
            # If both prediction and ground truth are empty, IoU is 1
            iou_values[b] = 1.0 if intersection == 0 else 0.0
    
    # Return mean IoU across the batch
    return iou_values.mean()
