...
def snapshot_matplotlib(mesh, angle, img_res, elev=30):
    # 1) Prepare figure & axes
    fig = plt.figure(figsize=(img_res/100, img_res/100), dpi=100)
    ...
    # 3) Build per-face RGBA colors
    base_rgb  = mesh.visual.vertex_colors[0][:3] / 255.0
    # find any annotated vertex to grab its color:
    annot_idx = np.where(~np.all(mesh.visual.vertex_colors[:,:3] == mesh.visual.vertex_colors[0,:3], axis=1))[0]
    annot_rgb = mesh.visual.vertex_colors[annot_idx[0]][:3] / 255.0

    facecolors = np.tile(base_rgb, (faces.shape[0], 1))
    contact_verts = set(annot_idx.tolist())
    mask = [bool(contact_verts.intersection(face)) for face in faces]
    facecolors[mask] = annot_rgb

    # append alpha channel
    facecolors_rgba = np.hstack([facecolors, np.ones((faces.shape[0],1))])

    # 4) Create the Poly3DCollection
    face_verts = verts[faces]  # (n_faces, 3, 3)
    ...
    # 7) Grab the RGB buffer
    fig.canvas.draw()
    H, W = fig.canvas.get_width_height()
    buf4 = np.frombuffer(fig.canvas.buffer_rgba(), dtype=np.uint8).reshape(H, W, 4)
    buf  = buf4[...,:3]
    plt.close(fig)

    return buf

def create_scene_cpu(mesh, img):
    """
    stack the input image plus 6 views of the mesh into one wide PNG,
    without ever using OpenGL/EGL.
    """
    img_res = 500
    # 1) resize input image to same height
    h = img_res
    w = int(h * img.shape[1] / img.shape[0])
    __ = cv2.resize(img, (w, h))[:, :, ::-1]  # BGR→RGB
    mesh_images = [__]

    # 2) four side views (azimuths 0,90,180,270)
    for az in (0, 90, 180, 270):
        img_cpu = snapshot_matplotlib(mesh, az, img_res)
        mesh_images.append(img_cpu)

    # 3) two top/bottom views (elevation +90, −90; use az=0)
    for el in (90, -90):
        # we cheat by rotating the mesh instead of changing elev:
        m2 = mesh.copy()
        R = trimesh.transformations.rotation_matrix(
                np.radians( el ), [1,0,0])
        m2.apply_transform(R)
        img_cpu = snapshot_matplotlib(m2, 0, img_res)
        mesh_images.append(img_cpu)

    # 4) stitch horizontally
    full = np.hstack(mesh_images)
    pil = pil_img.fromarray(full)
...
def main(args):
    ...
    for img_name in images:
        ...
        rend = create_scene_cpu(body_model_smpl, img)
...
if __name__=='__main__':
    ...
    parser.add_argument('--annot_colour', help='Colour of the annotation', nargs='+', type=int, default=[0, 255, 0, 255])
