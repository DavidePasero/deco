...
class DECO(nn.Module):
    def __init__(self, encoder='hrnet', context=True, device='cuda', use_transformer=True, num_object_classes=80):
        ...
        if self.encoder_type == 'hrnet':
            ...
            # Modified classifier initialization
            self.classif = Classifier(480, 6890, num_object_classes, use_transformer).to(device)
            ...
            # Modified classifier initialization for swin
            self.classif = Classifier(1024, 6890, num_object_classes, use_transformer).to(device)
        else:
    ...
    def forward(self, img):
        ...
        if self.context: 
            if self.use_transformer:
                # cont is now [B, C, V] for transformer case
                return cont, sem_mask_pred, part_mask_pred
            else:
