...
def train(hparams):
    # Initialize DECO model with transformer parameters
    deco_model = DECO(
        hparams.TRAINING.ENCODER, 
        hparams.TRAINING.CONTEXT, 
        device,
        use_transformer=hparams.TRAINING.USE_TRANSFORMER,
        num_object_classes=hparams.TRAINING.NUM_OBJECT_CLASSES
    )

    # Initialize TrainStepper with transformer parameters
    solver = TrainStepper(
        deco_model, 
        hparams.TRAINING.CONTEXT, 
        hparams.OPTIMIZER.LR, 
        hparams.TRAINING.LOSS_WEIGHTS, 
        hparams.TRAINING.PAL_LOSS_WEIGHTS, 
        device,
        use_transformer=hparams.TRAINING.USE_TRANSFORMER,
        num_object_classes=hparams.TRAINING.NUM_OBJECT_CLASSES
    )
